"""Dependency-free primitives for safely serialized static HTML reports."""
...
def render_document(*, title: str, main: Element, assets: TrustedAssets, language: str) -> str:
    for name, script in (
        ("syntax highlighter", assets.syntax_script),
        ("report behavior", assets.behavior_script),
    ):
        if SCRIPT_END_RE.search(script):
            raise ValueError(f"trusted {name} asset contains a closing script tag")

    root = Element("html", {"lang": language})
