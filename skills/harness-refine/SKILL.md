...
2. **채팅에는 후보당 세 줄로 요약**: `수정 대상`, `바꿀 내용`, `근거`를 각각 한 줄로 쓴다. 구체적인 형식은 `references/output-schema.md`를 따른다.
   - 수정 대상은 `메모리`, `스킬`, `익스텐션`, `도구`, `프로젝트 지침`처럼 종류부터 밝힌다. 이름·파일·섹션은 생략하거나 필요한 경우에만 괄호 안에 쓴다.
   - 바꿀 내용은 어떤 동작이나 규칙을 어떻게 바꿀지 한 문장으로 요약한다. 별도 `문제` 항목이나 배경 문단은 붙이지 않는다.
   - 근거는 관찰한 사실과 기대 효과를 연결해 한 문장으로 요약한다. 검증되지 않은 효과는 확정된 것처럼 쓰지 않는다. 상세 설명은 리포트에 둔다.
   - Coverage, metrics, Rejected signals, Evidence ID는 채팅에 출력하지 않는다. 리포트 경로를 함께 제공한다.
   - 후보 요약 뒤 마지막 섹션으로 `메모리 승격·폐기 제안`을 항상 출력한다. 각 제안도 같은 세 줄 형식을 쓰며, 삭제 전 확인 조건은 `바꿀 내용` 한 줄에 포함한다. 없으면 `없음`이라고 쓴다.
...
- 풀 리포트를 저장하고, 채팅의 각 후보를 종류부터 밝힌 수정 대상·바꿀 내용·근거의 세 줄로 요약했는가
