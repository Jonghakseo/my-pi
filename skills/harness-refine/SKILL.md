...
2. **채팅에는 적용 판단에 필요한 설명을 출력**: target 종류별로 묶고, 후보마다 `수정 대상`, `문제`, `바꿀 내용`, `효과를 기대하는 근거`를 짧게 설명한다. 구체적인 형식은 `references/output-schema.md`를 따른다.
   - 리포트를 열지 않아도 어떤 익스텐션·스킬·도구·지침을 어떻게 바꾸자는지 알 수 있어야 한다. 대상의 이름과 파일/섹션을 명시하고, 현재 동작과 제안한 동작의 차이를 쓴다.
   - 근거는 문제를 반복하는 대신 관찰한 결과와 수정안의 작동 방식을 연결한다. 이미 검증된 효과와 아직 기대하는 효과를 구분한다.
   - 한 줄 요약을 강제하지 않는다. 후보당 네 항목을 기본으로 쓰되 각각 1~2문장으로 제한한다. 상세 로그나 통계로 설명을 대신하지 않는다.
   - Coverage, metrics, Rejected signals, Evidence ID는 채팅에 출력하지 않는다. 리포트 경로를 함께 제공한다.
   - 후보 설명 뒤 마지막 섹션으로 `메모리 승격·폐기 제안`을 항상 출력한다. 각 제안도 대상·문제·변경·근거를 설명하고, 삭제 전 확인할 조건을 포함한다. 없으면 `없음`이라고 쓴다.
...
- 풀 리포트를 저장하고, 채팅만 읽어도 각 후보의 수정 대상·문제·구체적 변경·효과를 기대하는 근거를 알 수 있는가
- 해결됐다고 확인한 사실과 아직 검증하지 않은 기대 효과를 구분했는가
