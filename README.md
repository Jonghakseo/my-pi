...
| ⚡ **worker** | `claude-sonnet-4-6` | 범용 작업 실행기 | 구현, 작성, 수정 (복잡한 다중 파일) |
| 🏃 **worker-fast** | `claude-haiku-4-5` | 경량 단순 작업 실행기 | 단일 파일 수정, 간단한 변경 |
| 📐 **planner** | `gpt-5.3-codex` | 구현 설계자 | 복잡한 작업 분할 |
| 🔎 **reviewer** | `gpt-5.3-codex` | 코드 리뷰 (P0–P3 심각도) | PR 리뷰, 품질 점검 |
| 🥊 **challenger** | `claude-sonnet-4-6` | 스트레스 테스터 | 실행 전 계획 검증 |
| ✅ **verifier** | `claude-sonnet-4-6` | 3단계 근거 검증 | 주장 확인, 정확성 점검 |
| 🔬 **deep-verify** | `gpt-5.3-codex` | 고위험 심층 검증 | 삭제·배포 등 위험 작업 전 검증 |
| ⚖️ **decider** | `gpt-5.3-codex` | 기술 의사 결정 | 아키텍처 선택, 트레이드오프 분석 |
| 🌐 **searcher** | `claude-sonnet-4-6` | 리서치·웹 검색 | 문서 탐색, 조사 |
| 🖥️ **browser** | `claude-sonnet-4-6` | 브라우저 자동화·UI 테스트 | E2E 테스트, 시각 검증 |

<details>
<summary><strong>모델 선택 기준</strong></summary>

- **claude-haiku-4-5** — 초고속 경량 작업 (단순 단일 파일 변경)
- **claude-sonnet-4-6** — 범용 작업 (파일 탐색, 빠른 조회, 구현, 분석, 창작)
- **gpt-5.3-codex** — 구조적 추론 작업 (설계, 리뷰, 의사 결정, 심층 검증)

오케스트레이터(메인 에이전트)는 `claude-sonnet-4-6` 기반으로 동작하여 위임 결정을 수행한다.
