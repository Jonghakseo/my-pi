...
import type { Session, SessionInfo, SessionManager } from "./session-manager.js";
...
function isSessionScopedMessage(message: BrowserMessage): message is Extract<BrowserMessage, { sessionId: string }> {
	...
	const registerSession = (session: Session): void => {
		...
		const removeDataListener = session.onData((data, offset) => {
			...
			for (const viewer of session.getLiveViewers()) {
				...
				send(viewer, { type: "data", data, offset, sessionId: session.id, resumeId: state.resumeCounter }, true);
			}
		});
