...
async function getAuthToken(server: LockfileData): Promise<string | undefined> {
	...
	const response = await httpPost(
		`${protocol}://127.0.0.1:${server.port}/api/auth`,
		JSON.stringify({ pin: server.pin }),
		{
...
async function deleteRemoteSession(server: LockfileData, sessionId: string, authToken?: string): Promise<boolean> {
	...
	try {
		await httpDelete(
			`${protocol}://127.0.0.1:${server.port}/api/sessions/${encodeURIComponent(sessionId)}${tokenQuery}`,
			headers,
		);
		return true;
	} catch {
