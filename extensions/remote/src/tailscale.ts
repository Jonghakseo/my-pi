...
export async function getTailscaleInfo(): Promise<TailscaleInfo> {
  try {
    ...
    return {
      ...
      certDomains,
      funnelAvailable:
        Boolean(parsed.Capabilities?.["https"]) ||
...
export async function startFunnel(port: number): Promise<string> {
  await withRetry(
    () => execFileAsync("tailscale", ["funnel", "--bg", `http://127.0.0.1:${port}`], { encoding: "utf-8", timeout: 15_000 }),
  ...
  try {
    ...
    const parsed = JSON.parse(stdout) as { ServeConfig?: { TCP?: Record<string, { HTTPS?: boolean }> }; Funnel?: Record<string, unknown> };
