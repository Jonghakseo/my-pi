...
let onAuthRevokeCb;
let onPinChangeCb;
let failedAttemptsByIp = new Map();
let funnelAttempts = [];
...
export function registerAuthRevokeHandler(cb) {
    onAuthRevokeCb = cb;
}
...
export function generatePin(mode) {
    if (mode === "funnel") {
...
export function verifyPin(pin, remoteIp) {
    ensurePinFresh();
    if (currentMode === "lan") {
...
export async function wsAuthCheck(req) {
    ensurePinFresh();
    if (currentMode === "lan") {
...
export async function httpAuthMiddleware(req, res) {
    ensurePinFresh();
    if (currentMode === "lan") {
        if (isLocalRequest(req)) {
            return true;
...
export function resetPinAndSessions(reason) {
    if (currentMode === "lan") {
        lanToken = randomBytes(16).toString("hex");
        return null;
    }
...
function rotatePin(reason) {
    currentPin = generatePin(currentMode);
...
function clearPinRotationTimer() {
    if (pinRotationTimer) {
...
function getRemoteIdentity(remoteIp) {
...
async function readJsonBody(req, maxSize = 4096) {
    const chunks = [];
    let totalSize = 0;
