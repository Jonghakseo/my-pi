...
import { compile, compileRanked } from "../src/core/summarize.ts";
import { rankBriefBlocks, selectRankedBriefBlocks } from "../src/core/rank.ts";
...
describe("section-aware brief ranking prototype", () => {
	...
	it("penalizes scaffolding-only bash so it ranks below substantive commands", () => {
		const blocks: NormalizedBlock[] = [
			{ kind: "bash", command: "set -euo pipefail\ncd /tmp/proj\nls -la", output: "", exitCode: 0 },
			{ kind: "bash", command: 'set -euo pipefail\ngit commit -m "fix"\ngit push', output: "", exitCode: 0 },
		];
		const ranked = rankBriefBlocks(blocks);
	...
	it("does not penalize a scaffolding command that failed (nonzero exit is a real fact)", () => {
		const blocks: NormalizedBlock[] = [{ kind: "bash", command: "cd /tmp/missing", output: "", exitCode: 1 }];
		const ranked = rankBriefBlocks(blocks);
		expect(ranked[0].reasons).not.toContain("trivial-bash");
	...
	it("selects ranked blocks chronologically after scoring", () => {
		...
		expect(selected).toEqual([blocks[2], blocks[3], blocks[4], blocks[5]]);
	});
	...
	it("deduplicates repeated gh PR polling commands", () => {
		...
		const pollCount = selected.filter(
			(block) =>
	...
	it("keeps the ranked brief within the char budget even when recent turns are huge", () => {
		const huge = Array.from({ length: 10 }, (_, i) => assistantText(`huge recent turn ${i} ${"y".repeat(900)}`));
		const maxBriefChars = 3000;
	...
	it("ignores the size-relative ceiling unless slope and floor are both set", () => {
		...
		const flat = compileBrief(
			selectRankedBriefBlocks(blocks, { maxBlocks: 500, preserveRecentBlocks: 0, maxBriefChars: 4400 }),
		);
	...
	// 이 포크의 개선: [Session Goal] 추출이 실패하는 경우(URL 시작 지시문 등)에도
	// 세션 서두 사용자 메시지가 랭킹 컷에 잘려 유실되지 않게 한다.
	it("keeps the opening user block even when it loses the score cut", () => {
		const blocks: NormalizedBlock[] = [
			{ kind: "user", text: "로그인 버그를 고쳐줘" },
			...Array.from({ length: 120 }, (_, i) => ({
				kind: "assistant" as const,
				text: `work block ${i} ${"y".repeat(280)}`,
			})),
		];
		const selected = selectRankedBriefBlocks(blocks, { maxBlocks: 40, preserveRecentBlocks: 8 });
		expect(selected[0]).toBe(blocks[0]);
	});

	it("charges the forced opener against the char budget", () => {
		const blocks: NormalizedBlock[] = [
			{ kind: "user", text: `opener ${"z".repeat(220)}` },
			...Array.from({ length: 30 }, (_, i) => ({
				kind: "assistant" as const,
				text: `recent block ${i} ${"w".repeat(900)}`,
			})),
		];
		const maxBriefChars = 3000;
		const rendered = compileBrief(
			selectRankedBriefBlocks(blocks, { maxBlocks: 80, preserveRecentBlocks: 30, maxBriefChars }),
		);
		expect(rendered.length).toBeLessThanOrEqual(maxBriefChars);
		expect(rendered).toContain("opener");
	});
});
