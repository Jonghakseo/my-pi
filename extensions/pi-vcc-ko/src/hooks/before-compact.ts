...
export const formatCompactionStats = (stats: CompactionStats): string => {
	if (stats.budgetCut) {
		...
		return `pi-vcc-ko: kept ~${formatTokens(stats.keptTokensEst)} tok tail (mid-turn cut, ${reason}), summarized ${stats.summarized}.`;
	}
	const notes: string[] = [`summarized ${stats.summarized}`];
	if (stats.smartKeepAdjusted) {
		notes.push("smart-keep");
	}
	return `pi-vcc-ko: kept ${stats.keptUserTurns}/${stats.totalUserTurns} turns, ~${formatTokens(stats.keptTokensEst)} tok (${notes.join(", ")}).`;
};
...
const dbg = (settings: PiVccSettings, data: Record<string, unknown>) => {
	...
	try {
		writeFileSync("/tmp/pi-vcc-ko-debug.json", JSON.stringify(data, null, 2));
	} catch {}
};
...
export const findBudgetCutIndex = (live: EntryWithMessage[], maxTokens: number, charsPerToken?: number): number => {
	let acc = 0;
	let crossed = -1;
...
const tailTokensForKeep = (branchEntries: any[], keepUserTurns: number, charsPerToken?: number): number | null => {
	...
	const chars = kept.reduce((sum: number, e: any) => sum + estimateMessageContentChars(e.message?.content), 0);
...
const REASON_MESSAGES: Record<OwnCutCancelReason, string> = {
	no_live_messages: "pi-vcc-ko: Nothing to compact (no live messages)",
	too_few_live_messages: "pi-vcc-ko: Too few messages to compact",
};
...
export const registerBeforeCompactHook = (pi: ExtensionAPI, piVersion: string = VERSION) => {
	...
	pi.on("session_before_compact", (event, ctx) => {
		...
		// 사용자 주입 디노이즈 규칙을 해석한다. 잘못된 정규식은 경고로 알리고 나머지는 계속한다.
		const { rules: denoiseRules, errors: ruleErrors } = compileRules(settings.rules, settings.disableBuiltinRules);
		if (ruleErrors.length > 0) {
			try {
				ctx?.ui?.notify?.(`pi-vcc-ko: 무효 규칙 ${ruleErrors.length}건 건너뜀 — ${ruleErrors[0]}`, "warning");
			} catch {}
		}

		// Always handle explicit /pi-vcc-ko marker.
		...
		// because the model can change mid-session via /model. Explicit /pi-vcc-ko
		// bypasses the skip; undefined model never skips.
		const provider = (ctx as any)?.model?.provider;
		if (
			!isPiVcc &&
			typeof provider === "string" &&
			settings.skipForProviders.some((p) => p.toLowerCase() === provider.toLowerCase())
		)
			return;
		...
		if (!ownCut.ok) {
			...
			const lastKeptId: string | undefined = (lastComp as { firstKeptEntryId?: string } | undefined)?.firstKeptEntryId;
			...
			for (let i = 0; i < liveRoles.length; i++) {
				if (liveRoles[i] === "user") userIndices.push(i);
			}
			...
			dbg(settings, {
				...
				lastCompaction: lastComp
					? {
							hasFirstKeptEntryId: !!(lastComp as { firstKeptEntryId?: string }).firstKeptEntryId,
							foundInBranch: (lastComp as { firstKeptEntryId?: string }).firstKeptEntryId
								? (branchEntries as any[]).some(
										(e: any) => e.id === (lastComp as { firstKeptEntryId?: string }).firstKeptEntryId,
									)
								: null,
						}
					: null,
	...
	// /pi-vcc-ko path uses its own onComplete callback in the command handler.
	pi.on("session_compact", async (event, ctx) => {
		...
		// pi-vcc stamps details.compactor = "pi-vcc-ko" on its own compactions.
		if ((event as any).compactionEntry?.details?.compactor !== "pi-vcc-ko") return;
		const followUpPrompt = pendingFollowUpPrompt;
		pendingFollowUpPrompt = null;
		if (lastCompactWasPiVcc) return; // /pi-vcc-ko handles its own toast via onComplete
		if (willRetry) return;
