...
const TEST_COMMAND_RE =
	/\b(?:bun|npm|pnpm|yarn|node|pytest|cargo|go|mvn|gradle)\b[^\n]*(?:test|spec|check|lint|build|tsc)/i;
...
export const selectRankedBriefBlocks = (
	...
	// Non-null whenever `costs` is non-null (both derive from the same budget
	// decision). Captured as a local so loops read without `!`.
	const charBudget = maxBriefChars;
	...
	const costs =
		maxBriefChars == null ? null : blocks.map((b) => (b.kind === "tool_result" ? 0 : compileBrief([b]).length + 1));
	let usedChars = 0;

	// 세션 서두 사용자 메시지는 랭킹 점수와 무관하게 브리프에 남긴다 (예산은 차감).
	// [Session Goal] 추출이 실패해도 (URL 시작 지시문, 200자 초과 줄 등) 사용자 의도가
	// 요약에서 통초로 유실되지 않게 한다. 이 포크의 개선: 원본은 랭킹 컷에 그대로 잘렸다.
	if (blocks[0]?.kind === "user") {
		selected.add(0);
		if (costs && charBudget != null) usedChars += costs[0];
		const key = dedupKey(blocks[0]);
	...
	for (let i = blocks.length - 1; i >= Math.max(0, blocks.length - preserveRecentBlocks); i--) {
		...
		if (costs && charBudget != null && usedChars + costs[i] > charBudget) continue;
		selected.add(i);
	...
	for (const item of ordered) {
		...
		if (costs) {
			...
			if (usedChars + costs[item.index] > (charBudget ?? Number.POSITIVE_INFINITY)) continue;
