...
// 한국어 자기언급 접두어 (음, 어, 잠깐, 그런데 등). 공백/문장부호가 뒤따라야
// 매치하므로 "어제" 같은 실제 단어 시작은 보호된다.
const SELF_TALK_PREFIX_RE_KO = /^\s*(?:으?음+|아+|어+|그러니까|그니까|그러네|근데|그런데|잠깐|그래|아니)[,.!\s-]+/;
...
const STOP_WORDS = new Set([
	...
	"just",
	"also",
	// 한국어 기능어 (조사·접속부사·지시어). Intl.Segmenter가 조사를 분리해내는
	// 경우가 많아 붙어 있을 때보다 토큰 예산이 정확해진다.
	"이",
	"가",
	"은",
	"는",
...
const BODY_NOISE_RE =
	/^(?:import\s|from\s+\S+\s+import|require\(|const\s+\w+\s*=\s*require|#|\/\/|"""|'''|"use strict"|use\s+strict|<\?php)/;
	...
	for (let blockIndex = 0; blockIndex < blocks.length; blockIndex++) {
		...
		switch (b.kind) {
			...
			case "assistant": {
				...
				for (let i = 0; i < 2; i++) {
					const stripped = raw.replace(SELF_TALK_PREFIX_RE, "").replace(SELF_TALK_PREFIX_RE_KO, "");
