...
import { buildPiVccCustomInstructions, parseKeepAndPrompt } from "../core/compact-args.ts";
import { getLastCompactionStats, scheduleCompactionStatsNotify } from "../hooks/before-compact.ts";

export const registerPiVccCommand = (pi: ExtensionAPI) => {
	pi.registerCommand("pi-vcc-ko", {
		description: "Compact conversation with pi-vcc-ko structured summary",
			...
			ctx.compact({
				...
				onComplete: () => {
					...
					if (stats) {
						...
						ctx.ui.notify("Compacted with pi-vcc-ko", "info");
					}
					if (followUpPrompt) {
						try {
