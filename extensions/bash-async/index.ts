...
export default function bashAsync(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Do not call sleep or poll status, output, or list to wait. Continue only with independent work; otherwise end the turn. Every terminal result arrives automatically as a follow-up; killed jobs and results already read via status or output are not re-reported.",
		...
		async execute(_toolCallId, args, signal, _onUpdate, context) {
			...
			return execute(manager, notifications, args as BashAsyncParams, context, signal);
		},
	});
...
async function execute(
	...
	if (params.action === "status") {
		...
		if (isTerminalJobStatus(details.status)) notifications.acknowledge(params.jobId);
	...
	if (params.action === "output") {
		...
		if (isTerminalJobStatus(output.job.status)) notifications.acknowledge(params.jobId);
