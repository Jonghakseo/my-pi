...
export const TOOL_DESCRIPTION = `Run a finite, non-interactive shell command in the background. Use bash_async start for commands expected to take more than 30 seconds when the next action does not need their result immediately. Do not poll repeatedly, completion is delivered automatically. Query output only when early output is needed or the user asks. TUI, REPL, and commands requiring stdin are unsupported.`;
