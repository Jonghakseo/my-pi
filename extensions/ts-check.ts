 ...
 *
 * Original: https://github.com/joelhooks/pi-tools/blob/main/ts-check/ts-check.ts
 */
...
import { formatDiagnostics, langId, parseJsonRpcMessages, routeMessage } from "./utils/ts-check-utils.js";
...
function handleData(server: LSPServer, chunk: string) {
	const messages = parseJsonRpcMessages(server, chunk);
	for (const msg of messages) {
		routeMessage(server, msg);
	}
}
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("agent_end", async (_event, ctx) => {
		...
		const { text: report, totalErrors } = formatDiagnostics(server, root);
		const theme = ctx.ui.theme;
		if (report) {
			ctx.ui.setWidget("ts-check", [
				theme.fg("error", "●") +
					" " +
					theme.fg("dim", `tsgo: ${totalErrors} issue${totalErrors > 1 ? "s" : ""}`),
			]);
			ctx.ui.notify(
				`TypeScript issues:\n${report.split("\n").slice(0, 8).join("\n")}`,
				"warning",
			...
			ctx.ui.setWidget("ts-check", [
				theme.fg("success", "●") + " " + theme.fg("dim", "tsgo: clean"),
			]);
	...
	pi.registerTool({
		...
		description:
			"Query tsgo LSP — hover, definitions, references, diagnostics for TypeScript/JavaScript files. Powered by TypeScript 7 native compiler.",
		parameters: Type.Object({
			...
			line: Type.Optional(
				Type.Number({ description: "1-indexed line number (for hover/definition/references)" }),
			),
			column: Type.Optional(
				Type.Number({ description: "1-indexed column (for hover/definition/references)" }),
			),
		}),
		async execute(_id, params) {
			...
			if (!root)
				return {
					content: [{ type: "text" as const, text: "No tsconfig.json found" }],
					details: {},
				};
			...
			if (!server)
				return {
					content: [{ type: "text" as const, text: "Failed to start tsgo LSP" }],
					details: {},
				};
			...
			try {
				switch (params.action) {
					case "hover": {
						const r = await send(server, "textDocument/hover", {
							textDocument: { uri },
							position: pos,
						});
						if (!r || !r.contents)
					...
					case "definition": {
						const r = await send(server, "textDocument/definition", {
							textDocument: { uri },
							position: pos,
						});
					...
					case "references": {
						...
						const text = r
							...
							.join("\n");
						return {
							content: [{ type: "text" as const, text: `${r.length} references:\n${text}` }],
							details: {},
						};
					}
					case "diagnostics": {
						...
						const { text: report } = formatDiagnostics(server, root);
						return {
							content: [{ type: "text" as const, text: report || "No diagnostics" }],
							details: {},
						};
					}
					case "symbols": {
						const r = await send(server, "textDocument/documentSymbol", {
							textDocument: { uri },
						});
						if (!r || r.length === 0)
							return {
						...
						return {
							content: [
								{ type: "text" as const, text: r.map((s: any) => fmt(s)).join("\n") },
							],
							details: {},
						};
					}
				...
				return {
					content: [{ type: "text" as const, text: `LSP error: ${e.message || e}` }],
					details: {},
				};
			}
			return {
