...
import { CONFIG_PATH, loadConfigSection, normalizeApiKey } from "./config.js";
...
const PERPLEXITY_DEFAULTS: PerplexityConfig = { perplexityApiKey: null };

function loadPerplexityConfig(): PerplexityConfig {
	return loadConfigSection("perplexity", PERPLEXITY_DEFAULTS, (raw) => ({
		perplexityApiKey: normalizeApiKey(raw.perplexityApiKey),
	}));
}

function getApiKey(): string {
	const config = loadPerplexityConfig();
	const key = normalizeApiKey(process.env.PERPLEXITY_API_KEY) ?? config.perplexityApiKey;
	if (!key) {
		throw new Error(
...
export function isPerplexityAvailable(): boolean {
	const config = loadPerplexityConfig();
	return !!(normalizeApiKey(process.env.PERPLEXITY_API_KEY) ?? config.perplexityApiKey);
}
