...
> TUI에서는 메인 에이전트가 지휘하고 subagent가 따로 실행된 뒤 follow-up으로 결과를 돌려준다. non-TUI/API 환경에서는 subagent 완료 결과가 도구 응답으로 직접 반환된다.
...
- **모드별 실행**: TUI interactive에서는 launch 후 자동 follow-up을 쓰는 비동기 실행을 기본으로 두고, non-TUI/API/print/RPC에서는 완료까지 기다린 뒤 도구 응답으로 최종 결과를 반환한다.
...
현재 활성 상태 UI는 **above-editor run status widget**입니다.

- 활성 위젯: `widget.ts`
- legacy above-editor pixel widget: `above-widget.ts`에서 비활성화됨

즉 현재 동작 기준으로는 "픽셀 위젯"보다 **에디터 위 run 상태 박스**가 맞는 설명입니다.
