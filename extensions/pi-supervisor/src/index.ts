...
const startSupervisionSchema = Type.Object({
	outcome: Type.String({
		description:
...
type StartSupervisionParams = Static<typeof startSupervisionSchema>;
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("turn_end", async (event, ctx) => {
		...
		let decision: Awaited<ReturnType<typeof analyze>> | null = null;
		try {
	...
	pi.on("agent_end", async (_event, ctx) => {
		...
		const decision = await analyze(
			...
			(accumulated) => {
				...
				updateUI(ctx, state.getState() ?? s, { type: "analyzing", turn: s.turnCount, thinking });
			},
		);
	...
	pi.registerCommand("supervise", {
		...
		// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: upstream command flow kept as-is for local fork parity.
	...
	pi.registerTool({
		...
		// biome-ignore lint/suspicious/noExplicitAny: cast needed for local TypeBox/pi type compatibility.
		parameters: startSupervisionSchema as any,
		execute: async (_toolCallId, params: StartSupervisionParams, _signal, _onUpdate, ctx) => {
			const text = (msg: string) => ({ content: [{ type: "text" as const, text: msg }], details: {} });

			// Guard: supervision already active — model cannot modify it
			if (state.isActive()) {
				const s = state.getState();
				if (!s) {
					return text("Supervision state is unavailable.");
				}
				return text(
