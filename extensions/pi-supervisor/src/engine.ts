...
function extractCompactionSummary(ctx: ExtensionContext): string | null {
	...
	for (const entry of ctx.sessionManager.getBranch()) {
		const summaryValue = typeof entry === "object" && entry && "summary" in entry ? entry.summary : undefined;
		if ((entry.type === "compaction" || entry.type === "branch_summary") && typeof summaryValue === "string") {
			summary = summaryValue; // keep overwriting — last one wins (most recent)
		}
	}
	return summary;
}
...
function extractAssistantText(content: unknown): string {
	...
	const textParts = content.filter(isTextBlock).map((b) => b.text);
