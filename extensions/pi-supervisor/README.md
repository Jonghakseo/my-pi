...
This repo includes a local personal copy at `extensions/pi-supervisor/`, so it is auto-discovered when loaded through `@jonghakseo/my-pi`.

For one-off local development, you can also load it directly:

```bash
pi -e ./extensions/pi-supervisor/src/index.ts
```
