...
- **Only the `subagent`, lightweight orchestration tools (`list-agents`, `read`, `find`, `grep`, `ls`, `AskUserQuestion`, `todo_write`), and memory tools (`remember`, `recall`, `forget`, `memory_list`) are allowed** in master mode.
