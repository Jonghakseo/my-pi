// Adapted from @benvargas/pi-claude-code-use (MIT)
// Source: https://github.com/ben-vargas/pi-packages/tree/main/packages/pi-claude-code-use
// See LICENSE in this directory for the original copyright.
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: upstream rule pipeline; refactor would diverge from @benvargas/pi-claude-code-use.
...
function transformPayload(raw: Record<string, unknown>, disableFilter: boolean): Record<string, unknown> {
	...
	if (isPlainObject(payload.tool_choice)) {
		...
		if (remapped === undefined) {
			// biome-ignore lint/performance/noDelete: payload is a transient deep clone; delete is required to omit the field from the serialized request.
 ...
 */
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: upstream alias activation state machine; refactor would diverge from @benvargas/pi-claude-code-use.
